  ...
  const handleSendMessage = async () => {
    ...
    try {
      const res = await fetch(
        `https://api.get-a-developer.buttnetworks.com/api/message-user/${profile._id}`,
        {
          method: "POST",
          headers: {
...
useEffect(() => {
  const fetchProfile = async () => {
    try {
      ...
      const res = await fetch(`https://api.get-a-developer.buttnetworks.com/api/profile/${params.id}`);
      const data = await res.json();
        ...
        if (profileData.user) {
          try {
            const statusRes = await fetch(
              `https://api.get-a-developer.buttnetworks.com/api/profilestatus/${profileData.user}`
            );
