...
const AllDevelopers = ({ searchQuery }) => {
  const [profiles, setProfiles] = useState([]);
  const [category, setCategory] = useState("Keep Exploring");
  const categories = [
    "Keep Exploring",
    "Web Development",
  ...
  useEffect(() => {
    const activeFilter = searchQuery || category;
    ...
    const fetchProfiles = async () => {
      try {
        ...
        if (data.success) {
          ...
          } else if (category !== "Keep Exploring") {
            filtered = data.data.filter(
      ...
      <section className="min-h-screen px-6 py-16 bg-[#121212] text-white">
        <div className="max-w-7xl mx-auto mb-12">
          ...
          <p className="text-gray-400 mt-2 text-lg">
            Discover developers that match your interests and skills.
          </p>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-stretch max-w-7xl mx-auto">
          <div className="flex flex-col space-y-3 w-full md:w-1/4 bg-gradient-to-br from-[#1e1e1e] to-[#161616] p-6 rounded-2xl shadow-lg border border-gray-800">
            <h3 className="text-lg font-semibold text-white mb-4 pb-4 border-b border-gray-700">
              Categories
            </h3>
            ...
            {profiles.length === 0 ? (
              ...
              >
                {profiles.map((profile) => (
                  <SwiperSlide key={profile._id} className="h-auto">
                    <div className="bg-gradient-to-br from-[#1e1e1e] to-[#161616] rounded-2xl p-6 shadow-lg border border-gray-800 hover:border-blue-500/50 hover:shadow-blue-500/20 transition-all duration-300 flex flex-col h-[350px]">
